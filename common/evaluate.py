    ...
    def evaluate(self):
        ...
        for episode in tqdm(range(self.eva_episode)):
            ...
            episode_info = {'T_mot': [], 'W_mot': [], 'mot_eff': [], 'P_mot': [],
                            ...
                            'cell_power_out': [], 'P_batt': [], 'tep_a': [], 'tep_c': [], 'dsoh': [], 'temp_cost': []}
            ...
            for episode_step in range(self.episode_step):
                ...
                if episode_step+1 == self.episode_step:
                    ...
                    TEP_C.append(max(episode_info['tep_c']))
                    TEMP_COST.append(sum(episode_info['temp_cost']))
                    max_tep_c = max(episode_info['tep_c'])
                    print('\nepi %d: travel %.3fkm, SOC %.4f, Bat-SOH %.6f, FCS-SOH %.6f, Max Tep_c %.2f'
                          %(episode, f_travel, soc, bat_soh, fcs_soh, max_tep_c))
        ...
        scio.savemat(self.save_path+'/TEP_C.mat', mdict={'TEP_C': TEP_C})
        scio.savemat(self.save_path+'/TEMP_COST.mat', mdict={'TEMP_COST': TEMP_COST})
